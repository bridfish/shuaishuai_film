...
FijkPanelWidgetBuilder fijkCustomerPanelBuilder(
    {Key key,
  ...
  void initState() {
    ...
    _currentPosSubs = player.onCurrentPosUpdate.listen((v) {
      ...
      if (_needClearSeekData) {
        widget.data.clearValue(_FijkPanel2._fijkViewPanelSeekto);
      }
      _needClearSeekData = false;
    });

    if (widget.data.contains(_FijkPanel2._fijkViewPanelSeekto)) {
      var pos =
          widget.data.getValue(_FijkPanel2._fijkViewPanelSeekto) as double;
  ...
  void onVerticalDragStartFun(DragStartDetails d) {
    if (d.localPosition.dx > panelWidth() / 2) {
      ...
      FijkVolume.getVol().then((v) {
        if (widget.data != null &&
            !widget.data.contains(_FijkPanel2._fijkViewPanelVolume)) {
          widget.data.setValue(_FijkPanel2._fijkViewPanelVolume, v);
        }
        setState(() {
          _volume = v;
      ...
      FijkPlugin.screenBrightness().then((v) {
        if (widget.data != null &&
            !widget.data.contains(_FijkPanel2._fijkViewPanelBrightness)) {
          widget.data.setValue(_FijkPanel2._fijkViewPanelBrightness, v);
        }
        setState(() {
          _brightness = v;
          _valController.add(v);
      ...
      child: FijkSlider(
        ...
        onChangeEnd: (v) {
          setState(() {
            ...
            widget.data.setValue(_FijkPanel2._fijkViewPanelSeekto, _seekPos);
